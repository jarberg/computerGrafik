    ...
    if (mode.selectedIndex === 0){
      ...
      skip = false;
        ...
        if(counter >=1){
          ...
          var circle = make_circle_array(t);
          for (let i = 0; i < circle.length; i++) {
            gl.bindBuffer(gl.ARRAY_BUFFER, buffers.vBuffer);
            gl.bufferSubData(gl.ARRAY_BUFFER, sizeof['vec2']*index, flatten(circle[i]));
            if(i!=0){
              var col = vec4(colors[colorMenu.selectedIndex]);
              gl.bindBuffer(gl.ARRAY_BUFFER, buffers.cBuffer);
              gl.bufferSubData(gl.ARRAY_BUFFER, sizeof['vec3']*index, flatten(col));
            }
            index++;
          }
...
function render(){
  ...
  for (var i = 0; i < circles.length; ++i) {
    gl.drawArrays(gl.TRIANGLE_FAN, circles[i], 53);
  }
}
function make_circle_array(New){
  console.log(last)
  let n = 50;
  var a = last[0]-New[0];
  var b = last[1]-New[1];
  var radius = Math.sqrt( a*a + b*b );
  var returnArray =[last];
  for (let i = 0; i < n+2; i++) {
    let angle = 2*Math.PI*i/n
    returnArray.push(vec2((radius*Math.cos(angle)+last[0]), (radius*Math.sin(angle)+last[1])));
  }
  return returnArray;
}
window.onload = main;
