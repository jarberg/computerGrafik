...
function render(){
  ...
  for (let i = 0; i < objects.length; i++) {
    ...
    if (! obj instanceof PointLight){
      ...
      m[3][3] = 0.0;
      m[3][1] = -1.0/(light.position[1]+0.01);

      var modelViewMatrix = mult(translate(-light.position[0], -light.position[1], -light.position[2]), obj.transformMatrix);
      modelViewMatrix = mult(m, modelViewMatrix);
      modelViewMatrix = mult(translate(light.position[0], light.position[1], light.position[2]), modelViewMatrix );

      // Send color and matrix for shadow
      gl.uniformMatrix4fv( gl.getUniformLocation(program,"modelViewMatrix"), false,
          flatten(modelViewMatrix));
      obj.draw(camera);
      obj.vertexColors =tempColors;
    }
